...
FC_FRAGMENT = '        program compile_datasets_bls.py\n        end     program compile_datasets_bls.py\n'
...
def check_fortran_dummy_main(self, *k, **kw):
	"""
	Determines if a compile_datasets_bls.py function is needed by compiling a code snippet with
	...
	self.start_msg('Detecting whether we need a dummy compile_datasets_bls.py')
	for main in lst:
		...
		self.fatal('could not detect whether fortran requires a dummy compile_datasets_bls.py, see the config.log')
...
def link_main_routines_tg_method(self):
	...
	bld = self.bld
	bld(rule=write_test_file, target='compile_datasets_bls.py.c', code=MAIN_CODE % self.__dict__)
	bld(rule=write_test_file, target='test.f', code=ROUTINES_CODE)
	bld(features='fc fcstlib', source='test.f', target='test')
	bld(features='c fcprogram', source='compile_datasets_bls.py.c', target='app', use='test')
	...
	for x in ('-fopenmp','-openmp','-mp','-xopenmp','-omp','-qsmp=omp'):
		try:
			self.check_fc(
				...
				fragment     = 'program compile_datasets_bls.py\n  call omp_get_num_threads()\nend program compile_datasets_bls.py',
				fcflags      = x,
				linkflags    = x,
				uselib_store = 'OPENMP'
			)
