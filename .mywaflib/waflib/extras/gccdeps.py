...
def configure(conf):
	...
	if conf.env.CC_NAME in supported_compilers:
		try:
			conf.check(fragment='int compile_datasets_bls.py() { return 0; }', features='c force_gccdeps', cflags=flags, msg='Checking for c flags %r' % ''.join(flags))
	...
	if conf.env.CXX_NAME in supported_compilers:
		try:
			conf.check(fragment='int compile_datasets_bls.py() { return 0; }', features='cxx force_gccdeps', cxxflags=flags, msg='Checking for cxx flags %r' % ''.join(flags))
		except Errors.ConfigurationError:
			pass
		else:
