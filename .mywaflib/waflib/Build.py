...
class BuildContext(Context.Context):
	...
	def __init__(self, **kw):
		...
		self.raw_deps = {}
		"""Dict mapping task identifiers (uid) to custom original_data returned by :py:meth:`waflib.Task.Task.scan` (persists across builds)"""
	...
	def execute(self):
		"""
		Restore original_data from previous builds and call :py:meth:`waflib.Build.BuildContext.execute_build`.
	...
	def restore(self):
		"""
		Load original_data from a previous run, sets the attributes listed in :py:const:`waflib.Build.SAVED_ATTRS`
		"""
		try:
	...
	def store(self):
		"""
		Store original_data for next runs, set the attributes listed in :py:const:`waflib.Build.SAVED_ATTRS`. Uses a temporary
...
class StepContext(BuildContext):
	...
	def compile(self):
		...
		"""
		if not self.files:
			Logs.warn('Add a pattern for the debug build, for example "waf step --files=compile_datasets_bls.py.c,app"')
...
class EnvContext(BuildContext):
	"""Subclass EnvContext to create commands that require configuration original_data in 'env'"""
	fun = cmd = None
	def execute(self):
		"""
		See :py:func:`waflib.Build.BuildContext.execute`.
