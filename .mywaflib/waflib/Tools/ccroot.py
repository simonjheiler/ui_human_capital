...
def apply_link(self):
	...
	matching a name from the attribute *features*, for example::
			...
			def build(bld):
				tg = bld(features='cxx cxxprogram cprogram', source='compile_datasets_bls.py.c', target='app')
...
def process_use(self):
	...
	Process the ``use`` attribute which contains a list of task generator names::
		...
		def build(bld):
			...
			bld.program(source='compile_datasets_bls.py.c', target='app', use='lib1')
...
def apply_implib(self):
	...
	if getattr(self, 'install_task', None):
		try:
			...
			try:
				# user has given an installation path for the compile_datasets_bls.py library, put the import library in it
				inst_to = self.install_path
			except AttributeError:
...
def read_shlib(self, name, paths=[], export_includes=[], export_defines=[]):
	...
	Read a system shared library, enabling its use as a local library. Will trigger a rebuild if the file changes::
		...
		def build(bld):
			bld.read_shlib('m')
			bld.program(source='compile_datasets_bls.py.c', use='m')
	"""
