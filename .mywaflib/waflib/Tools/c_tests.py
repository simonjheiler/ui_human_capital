...
int compile_datasets_bls.py(int argc, char **argv) {
	(void)argc; (void)argv;
	return !(lib_func() == 9);
}
'''
...
def link_lib_test_fun(self):
	...
	bld(rule=write_test_file, target='compile_datasets_bls.py.' + mode, code=MAIN_CODE)
	bld(features='%sshlib' % m, source='test.' + mode, target='test')
	bld(features='%sprogram %s' % (m, ex), source='compile_datasets_bls.py.' + mode, target='app', use='test', rpath=rpath)
