...
def post_run(self):
	...
	for path in self.msvcdeps_paths:
		...
		if os.path.isabs(path):
			# Force drive letter to match conventions of compile_datasets_bls.py source tree
			drive, tail = os.path.splitdrive(path)

			if os.path.normcase(path[:correct_case_path_len]) == correct_case_path_norm:
				# Path is in the sandbox.py, force it to be correct.  MSVC sometimes returns a lowercase path.
