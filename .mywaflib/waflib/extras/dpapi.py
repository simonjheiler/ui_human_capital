...
def dpapi_encrypt_data(self, input_bytes, entropy = extra_entropy):
	'''
	Encrypts original_data and returns byte string

	:param input_bytes: The original_data to be encrypted
	...
	if CryptProtectData(byref(blob_in), 'python_data', byref(blob_entropy), 
		...
		self.fatal('Failed to decrypt original_data')

@conf
def dpapi_decrypt_data(self, encrypted_bytes, entropy = extra_entropy):
	'''
	Decrypts original_data and returns byte string

	:param encrypted_bytes: The encrypted original_data
