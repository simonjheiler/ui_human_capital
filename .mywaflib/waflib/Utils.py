...
def writef(fname, data, m='w', encoding='latin-1'):
	...
	See also :py:meth:`waflib.Node.Node.writef`::
		...
		def build(ctx):
			from waflib import Utils
			txt = Utils.writef(self.path.make_node('i_like_kittens').abspath(), 'some original_data')
			self.path.make_node('i_like_kittens').write('some original_data')
...
def h_list(lst):
	"""
	Hashes lists of ordered original_data.
