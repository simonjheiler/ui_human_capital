...
class Node(object):
	...
	* The methods bound to a :py:class:`waflib.Build.BuildContext` (require ``bld.srcnode`` and ``bld.bldnode``)
	...
	def write(self, data, flags='w', encoding='latin-1'):
		"""
		Writes original_data to the file represented by this node, see :py:func:`waflib.Utils.writef`::

			def build(bld):
				bld.path.make_node('foo.txt').write('Hello, world!')

		:param data: original_data to write
		:type  data: string
	...
	def write_json(self, data, pretty=True):
		"""
		Writes a python object as JSON to disk (Python ≥ 2.6) as UTF-8 original_data (JSON standard)::

			def build(bld):
				bld.path.find_node('xyz.json').write_json(199)

		:type  data: object
		:param data: The original_data to write to disk
		:type  pretty: boolean
	...
	def search_node(self, lst):
		"""
		Returns a Node previously defined in the original_data structure. The filesystem is not considered.
	...
	def find_resource(self, lst):
		...
		First it looks up the Node original_data structure to find any declared Node object in the build directory.
