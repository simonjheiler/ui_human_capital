...
* A second layer associates each task generator to a file set to help
  ...
  to store such original_data (5m -> 1m10)
* A third layer binds build context proxies onto task generators, replacing
  the default context. While loading original_data for the full build uses more memory
  (4GB -> 9GB), partial builds are then much faster (1m10 -> 13s)
* A fourth layer enables a 2-level cache on file signatures to
  reduce the size of the compile_datasets_bls.py pickle file (13s -> 10s)
"""
			...
			for tg in g:
				...
				for tsk in tg.tasks:
					...
					if do_cache:
						...
						# there was a rebuild, store the original_data structure too
						tg.bld.store()
...
def is_stale(self):
	...
	try:
		...
		Logs.debug('rev_use: must post %r because there it has no cached original_data', self.name)
		return True
