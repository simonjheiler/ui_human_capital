...
The following snippet illustrates the tool usage::
	...
	def build(bld):
		bld(
			...
			source   = 'compile_datasets_bls.py.cpp textures.qrc aboutDialog.ui',
			target   = 'window',
		)
...
class qxx(Task.classes['cxx']):
	...
	def create_moc_task(self, h_node, m_node):
		...
		try:
			...
			# direct injection in the build phase (safe because called from the compile_datasets_bls.py thread)
...
def process_mocs(self):
	...
	Processes MOC files included in headers::
		...
		def build(bld):
			bld.program(features='qt5', source='compile_datasets_bls.py.cpp', target='app', use='QT5CORE', moc='foo.h')
...
def apply_qt5(self):
	...
	Adds MOC_FLAGS which may be necessary for moc::
		...
		def build(bld):
			bld.program(features='qt5', source='compile_datasets_bls.py.cpp', target='app', use='QT5CORE')
...
def configure(self):
	...
	frag = '#include <QApplication>\nint compile_datasets_bls.py(int argc, char **argv) {return 0;}\n'
	...
	if Utils.unversioned_sys_platform() == 'freebsd':
		frag = '#include <QApplication>\nint compile_datasets_bls.py(int argc, char **argv) { QApplication app(argc, argv); return NULL != (void*) (&app);}\n'
		try:
